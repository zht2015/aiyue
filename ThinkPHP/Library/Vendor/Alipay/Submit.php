...
class AlipaySubmit {
    ...
    function AlipaySubmit1($alipay_config) {
...
<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"><title>支付宝即时到账交易接口接口</title>
</head><body><form id='alipaysubmit' name='alipaysubmit' action='" . $this->alipay_gateway_new . "_input_charset=" . trim(strtolower($this->alipay_config['input_charset'])) . "' method='" . $method . "'>";
        while (list ($key, $val) = each($para)) {
            $sHtml.= "<input type='hidden' name='" . $key . "' value='" . $val . "'/>";
        }

        //submit按钮控件请不要含有name属性
        $sHtml = $sHtml . "<input type='hidden' value='" . $button_name . "'></form>正在向支付宝提交数据...</body></html>";
