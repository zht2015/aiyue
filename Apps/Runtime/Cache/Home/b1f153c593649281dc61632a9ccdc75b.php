...
<script type="text/javascript">
  ...
  function sendVerifyCode(){
    ...
    $.post("<?php echo U('Home/Users/sendMsg');?>", {"phone":phone,"templateId":31120,"type":"findPwd"}, function(data){
      if(data.status==1){
          curCount = count;
  ...
  function findPwd(){
    ...
    $.post("<?php echo U('Home/Users/forgetPassword');?>",{"phone":phone,"verifyCode":verifyCode},function(data) {
       
        if(data.status==1){
            //location.href="<?php echo U('Users/resetPassword');?>";
        }else if(data.status==-1){
           layer.tips(data.message, '#verifyCode', {
               tips: [1, '#F90'],
               time: 1000
