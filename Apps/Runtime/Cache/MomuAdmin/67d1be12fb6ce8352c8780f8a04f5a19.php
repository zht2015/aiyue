...
<body class="mainbody">
<form method="get" action="/index.php/MomuAdmin/Channel/insert_channel" id="form1">
<!--导航栏-->
<div class="location">
...
<div class="tab-content">
    ...
    <dd>
      <div class="rule-single-select">
        <select name="ddlSiteId" id="ddlSiteId" onchange="getChannelCoums();" datatype="*" errormsg="请选择所属站点！" sucmsg=" ">
			<option value="">请选择所属栏目</option>
			<option <?php if($partnum == '1'): ?>selected<?php endif; ?> value="1">新闻</option>
			<option <?php if($partnum == '2'): ?>selected<?php endif; ?> value="2">广告</option>
			<option <?php if($partnum == '3'): ?>selected<?php endif; ?> value="3">商城</option>
			<!-- <option <?php if($partnum == '4'): ?>selected<?php endif; ?> value="4">财务</option> -->
		</select>
      </div>
    </dd>
  </dl>
  ...
  <dl>
    <dt>调用名称</dt>
    <dd>
      <input name="txtName" type="text" id="txtName" class="input normal" datatype="/^[a-zA-Z0-9\-\_]{2,50}$/" errormsg="请填写正确的名称！" sucmsg=" " ajaxurl="" />
  ...
  <dl>
    <dt>频道标题</dt>
    <dd><input name="txtTitle" type="text" id="txtTitle" class="input normal" datatype="*2-100" sucmsg=" " /> <span class="Validform_checktip">*标题备注，允许中文。</span></dd>
  </dl>
  <!-- <dl>
    <dt>所属站点</dt>
    <dd>
      <div class="rule-single-select">
        <select name="ddlSiteId" id="ddlSiteId" datatype="*" errormsg="请选择所属站点！" sucmsg=" ">
			...
			<?php if(is_array($sites)): $i = 0; $__LIST__ = $sites;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
      </div>
    </dd>
  </dl> -->
  <dl>
    ...
    <dd>
      <div class="rule-single-checkbox">
          <input id="cbIsAlbums" type="checkbox" name="cbIsAlbums" />
      </div>
  ...
  <dl>
    ...
    <dd>
      <div class="rule-single-checkbox">
          <input id="cbIsAttach" type="checkbox" name="cbIsAttach" />
      </div>
  ...
  <!-- <dl>
    ...
    <dd>
      <div class="rule-single-checkbox">
          <input id="cbIsSpec" type="checkbox" name="cbIsSpec" />
      </div>
  ...
  <?php if(!empty($partnum)): if(!empty($coumsdata)): ?><dl>
			    <dt>选择字段</dt>
			    <dd>
			      <div class="rule-multi-porp">
			          <span id="cblAttributeField">
			          		<?php if(is_array($coumsdata)): $k = 0; $__LIST__ = $coumsdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><input id="cblAttributeField_<?php echo ($k); ?>" type="checkbox" name="cblAttributeField[]" value="<?php echo ($vo["id"]); ?>" />
  		...
  		<?php if(!empty($coumsdata)): ?><dl>
			    <dt>列表显示字段</dt>
			    <dd>
			      <div class="rule-multi-porp">
			          <span id="cblListField">
			          		<?php if(is_array($coumsdata)): $k = 0; $__LIST__ = $coumsdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><input id="cblListField_<?php echo ($k); ?>" type="checkbox" name="cblListField[]" value="<?php echo ($vo["id"]); ?>" />
			    ...
			    <dd>
			      <div class="rule-multi-porp">
			          <span id="cblSearchField">
			          		<?php if(is_array($coumsdata)): $k = 0; $__LIST__ = $coumsdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><input id="cblSearchField_<?php echo ($k); ?>" type="checkbox" name="cblSearchField[]" value="<?php echo ($vo["id"]); ?>" />
...
<script>
	function getChannelCoums(){
		...
		window.location.href="/index.php/MomuAdmin/Channel/channel_add/id/"+partnum;
		/*
		//清空字段内容
		$("#cblAttributeField").html('');
		if(partnum != '' && partnum != undefined){
			$.post("<?php echo U('Channel/getCoums');?>",{"id":partnum},function(data){
				var d = eval("("+data+")");
				var coumstr = "";
				if(d.code == 200){//有数据
					var objdata = d.data;
					$.each(objdata,function(n,v){
						coumstr += "<input id='cblAttributeField_"+n+"' type='checkbox' name='cblAttributeField[]' value='"+v['id']+"' />";
						coumstr += "<label for='cblAttributeField_"+n+"'>"+v['title']+"</label>";
					});
					
					//将数据插入html标签中
					$("#cblAttributeField").html(coumstr);
