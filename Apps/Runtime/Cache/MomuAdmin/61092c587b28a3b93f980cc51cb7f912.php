...
<body class="mainbody">
<form method="post" action="/index.php/MomuAdmin/Market/article_save/channel_id/<?php echo ($channelid); ?>/id/<?php echo ($eid); ?>" id="form1">
<input type="hidden" name="viewtype" value="<?php echo ($viewtype); ?>" />
<!--导航栏-->
<div class="location">
  <a href="/index.php/MomuAdmin/Market/article_list/channel_id/<?php echo ($channelid); ?>" class="back"><i></i><span>返回列表页</span></a>
  <a href="" class="home"><i></i><span>首页</span></a>
  <i class="arrow"></i>
  <a href="/index.php/MomuAdmin/Market/article_list/channel_id/<?php echo ($channelid); ?>"><span>内容管理</span></a>
  <i class="arrow"></i>
  <span>新增内容</span>
</div>
	        ...
	        <dd>
	        	<?php if(is_array($categorydata)): foreach($categorydata as $k=>$vo): ?><div class="rule-multi-porp multi-porps">
	        			<?php if(is_array($vo)): $i = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><span >
			                        <input id="ddlCategoryId_<?php echo ($v['title']); ?>" <?php echo (getChoseCategory($v['id'],$eid,3)); ?>  type="checkbox" name="ddlCategoryId[]" value="<?php echo ($v['id']); ?>" />
