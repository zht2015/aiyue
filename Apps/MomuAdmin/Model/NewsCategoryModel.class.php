...
class NewsCategoryModel extends CommonModel{
    ...
    public function getTabList($coum="",$order="id asc",$where="",$Page="",$type=""){
        return $this->getList("news_category",$coum,$order,$where,$Page,$type); 
    }
    ...
    public function dataSave(){
        ...
        $res = M("news_category")->where(array("id"=>$eid))->save($d);
        if($res === "false"){
            return false;
        }else{
            return true;
    ...
    public function delDataById($delstr){
        $tab1 = M("news_category");
        $tab2 = M("news_cat_relation");
        //开启事务
        $m = M();
        $m->startTrans();
