...
class AdvertisementCategoryModel extends CommonModel{
    /***
     * @time 2016/06/02
     * @biking
    ...
    public function getTabList($coum="",$order="id asc",$where="",$Page="",$type=""){
        return $this->getList("advertisement_category",$coum,$order,$where,$Page,$type); 
    }
    ...
    public function dataInsert(){
        ...
        $res = M("advertisement_category")->add($d);
        if($res === "false"){
            return false;
        }else{
            return true;
    ...
    public function dataSave(){
        ...
        $res = M("advertisement_category")->where(array("id"=>$eid))->save($d);
        if($res === "false"){
            return false;
        }else{
            return true;
    ...
    public function delDataById($delstr){
        $tab1 = M("advertisement_category");
        $tab2 = M("adv_cat_relation");
        //开启事务
        $m = M();
        $m->startTrans();
