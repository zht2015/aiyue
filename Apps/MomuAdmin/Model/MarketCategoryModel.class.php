...
class MarketCategoryModel extends CommonModel{
    /***
     * @time 2016/06/02
     * @biking
    ...
    public function getTabList($coum="",$order="id asc",$where="",$Page="",$type=""){
        return $this->getList("market_category",$coum,$order,$where,$Page,$type); 
    }
    ...
    public function dataInsert(){
        ...
        $d['is_select'] = isset($_REQUEST['isselect'])?I('isselect'):1;
        $res = M("market_category")->add($d);
        if($res === "false"){
            return false;
        }else{
            return true;
    ...
    public function dataSave(){
        ...
        $d['is_rec'] = isset($_REQUEST['isrec'])?I('isrec'):1;
        $d['is_select'] = isset($_REQUEST['isselect'])?I('isselect'):1;
        $res = M("market_category")->where(array("id"=>$eid))->save($d);
        if($res === "false"){
            return false;
        }else{
            return true;
    ...
    public function delDataById($delstr){
        $tab1 = M("market_category");
        $tab2 = M("market_cat_relation");
        //开启事务
        $m = M();
        $m->startTrans();
