...
class AdvertiseAction extends CommonAction {
    ...
    public function article_list() {
       ...
       if(IS_POST){
           ...
           if($evenname == "lbtnViewTxt"){
               ...
               if($evenname=="btnDelete"){
                   ...
                   $dres = D("AdvertisementValue")->delDataById($checkidstr);
                   $this->redirect("Advertise/article_list",array("channel_id"=>$this->channel_id,"flag"=>"2","viewtype"=>$defaultview,"txtPageNum"=>$pagenum));
               }
           }
       ...
       if($ddlCategoryId != "" && $ddlCategoryId != "0"){//筛选分类
           $articleidarr = D("AdvCatRelation")->getTabList(array("articleid"),"id asc",array("categoryid"=>$ddlCategoryId)); 
       ...
       if($defaultview == 'viewimg'){//列表视图模板
           $listcoums = D("AdvertisementField")->showListCoum($this->channel_id);
       }else{//图片视图模板
           $listcoums = '';
       }
       
        $count = D("AdvertisementValue")->getTabCount($where);// 查询满足要求的总记录数
        $Page = new \Think\Page($count,$pagenum);// 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show();// 分页显示输出
        $list = D("AdvertisementValue")->getTabList('',"id desc",$where,$Page);// 获取分页数据
    ...
    public function article_add(){
        ...
        $articlecate = array();
        //获取一级分类的数据
        $catdata = D('AdvertisementCategory')->getTabList(array('id','title'),'id asc',array('channel_id'=>$this->channel_id,'parent_id'=>0));

        //将一级分类和该一级分类下的二级分类数据全部写入到二维数组中
        foreach($catdata as $v){
            $tmp = array();
            array_push($tmp,array('id'=>$v['id'],'title'=>$v['title']));
            $subcatdata = D('AdvertisementCategory')->getTabList(array('id','title'),'id asc',array('parent_id'=>$v['id']));
    ...
    public function article_insert(){
        ...
        $res = D("AdvertisementValue")->articleDataInsert($this->channel_id);
        if($res == "false"){
            $this->error("新增失败了！请重试");die();
        }
        $this->redirect("Advertise/article_list",array("channel_id"=>$this->channel_id,"viewtype"=>$viewtype,"flag"=>"1"));
    ...
    public function article_edit(){
        ...
        //获取产品的相册信息
        $albums = D("AdvAlbums")->getTabList("","",array("article_id"=>$eid));
        //获取产品的附件信息
        $attachs = D("AdvAttach")->getTabList("","",array("article_id"=>$eid));
        //获取产品的扩展信息
        $extval  = D("AdvertisementValue")->getTabList("","",array("id"=>$eid),"","1");
    ...
    public function article_save(){
        ...
        $eid = isset($_REQUEST['id'])?$_REQUEST['id']:0;
        if($eid == 0){$this->error('获取不到需要编辑的模块！');die();}
        $res = D("AdvertisementValue")->articleDataSave($this->channel_id,$eid);
        if($res == "false"){
            //$this->error("编辑失败了！请重试");die();
            $this->redirect("Advertise/article_list",array("channel_id"=>$this->channel_id,"viewtype"=>$viewtype,"flag"=>"6"));
        }
        $this->redirect("Advertise/article_list",array("channel_id"=>$this->channel_id,"viewtype"=>$viewtype,"flag"=>"3"));
    }
    /***
     * @time 2016/06/24
     * @function 无限极分类列表
    ...
    public function category_list(){
        ...
        if(IS_POST){
            ...
            if($_REQUEST['__EVENTTARGET'] == "btnDelete"){
                ...
                if($flag){//可进行批量删除操作
                    $delres = D("AdvertisementCategory")->delDataById($delstr);
                    if($delres){
                        $this->redirect('Advertise/category_list',array("channel_id"=>$this->channel_id,"flag"=>"2"));
                    }else{
                        $this->redirect('Advertise/category_list',array("channel_id"=>$this->channel_id,"flag"=>"5"));
                    }
                }else{
                     $this->redirect('Advertise/category_list',array("channel_id"=>$this->channel_id,"flag"=>"7","msg"=>"批量删除失败!选中的分类不能包含子分类！请先将子分类进行删除后再将该父分类删除！"));die();
                }
    ...
    public function category_insert(){
        //var_dump($_REQUEST);die();
        $res = D("AdvertisementCategory")->dataInsert();
        if($res){
            $this->redirect('Advertise/category_list',array("channel_id"=>$this->channel_id,"flag"=>"1"));die();
        }
             $this->redirect('Advertise/category_list',array("channel_id"=>$this->channel_id,"flag"=>"4"));
    ...
    public function category_edit(){
        $eid = isset($_REQUEST['id'])?$_REQUEST['id']:0;
        if($eid == 0){$this->redirect('Advertise/category_list',array("channel_id"=>$this->channel_id,"msg"=>"获取不到需要编辑的行"));die();}
        $edata = D("AdvertisementCategory")->getTabList("","",array("id"=>$eid),"","1");
        //var_dump($edata);
    ...
    public function category_save(){
        //var_dump($_REQUEST);die();
        $res = D("AdvertisementCategory")->dataSave();
        if($res){
            $this->redirect('Advertise/category_list',array("channel_id"=>$this->channel_id,"flag"=>"3"));die();
        }
        $this->redirect('Advertise/category_list',array("channel_id"=>$this->channel_id,"flag"=>"6"));
    }
    ...
    public function getParentData($type=""){
        ...
        //1、获取所有的一级分类
        if(!empty($type)){
            $data = D("AdvertisementCategory")->getTabList(array("id","title"),"sort_id asc",array("channel_id"=>$this->channel_id,"parent_id"=>0));
        }else{
            $data = D("AdvertisementCategory")->getTabList("","sort_id asc",array("channel_id"=>$this->channel_id,"parent_id"=>0));
        }
    ...
    public function getAllSubCategory($pid,$leve,&$categorydata,$type=""){
        $strper = "&emsp;&emsp;";
        if(!empty($type)){
            $tmp = D("AdvertisementCategory")->getTabList(array("id","title"),"sort_id asc",array("channel_id"=>$this->channel_id,"parent_id"=>$pid));
        }else{
            $tmp = D("AdvertisementCategory")->getTabList("","sort_id asc",array("channel_id"=>$this->channel_id,"parent_id"=>$pid));
        }
        for($i=0;$i<$leve;$i++){
            $str.= $strper;
        }
    ...
    public function isHaveSubCategory($id){
        $res = D("AdvertisementCategory")->getTabList("","",array("parent_id"=>$id));
        if(count($res)>0){
            //存在
            return true;
        }
