...
function ExePostBack(objId, objmsg) {
    ...
    parent.dialog({
        ...
        ok: function () {
            __doPostBack(objId, '');
        	//alert("you click the ensure btn!");
        },
        cancelValue: '取消',
        cancel: function () { }
    }).showModal();
...
$.fn.ruleMultiPorp = function() {
	var multiPorp = function(parentObj){
		...
		parentObj.find(":checkbox").each(function(n){
			var indexNum = parentObj.find(":checkbox").index(this); //当前索引
			if(n == 0){
				var liObj = $('<li class="selectno"></li>').appendTo(divObj)
			}else{
				var liObj = $('<li></li>').appendTo(divObj)
			}
			if(n == 0){
				var newObj = $('<a data="f1" href="javascript:;">' + parentObj.find('label').eq(indexNum).text() + '</a><i></i>').appendTo(liObj); //查找对应Label创建选项
			}else{
...
//========================系统弹窗提示================================
function showResult(result,tips){
	var message = "error";
	if(tips != ""){
		message = tips;
	}else{
